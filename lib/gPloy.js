...
var configManager = require('./gPloy/configManager');
...
function gPloy() {
    ...
    this.killBackground = function() {
        configManager.getAllConfigs().forEach(function(config) {
            var pid = pidManager.getAll(config.name + "-forever");
            pid.forEach(function(p) {
                if (debug) {
                    console.log(" [DEBUG] Killed "+ config.name +"-forever: ".blue + p.toString().blue);
                }
                ...
                pidManager.remove(config.name + "-forever", p);
            });

            pid = pidManager.getAll(config.name);
            pid.forEach(function(p) {
                if (debug) {
                    console.log(" [DEBUG] Killed "+ config.name +": ".blue + p.toString().blue);
                }
                ...
                pidManager.remove(config.name, p);
            });
        });
